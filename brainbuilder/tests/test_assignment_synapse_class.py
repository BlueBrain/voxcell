...
def test_assign_sypnapse_class_empty():
    sclass = assign_synapse_class(np.zeros(shape=(0, 3)), inhibitory_proportion=0)
    eq_(sclass.shape, (0,))


def test_assign_sypnapse_class_all_excitatory():
    result = assign_synapse_class(np.zeros(shape=(10, 3)), inhibitory_proportion=0)
    eq_(result.shape, (10,))
    assert_equal(result, np.array(['excitatory'] * 10))


def test_assign_sypnapse_class_all_inhibitory():
    result = assign_synapse_class(np.zeros(shape=(10, 3)), inhibitory_proportion=1)
    eq_(result.shape, (10,))
