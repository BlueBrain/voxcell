...
(function() {
  ...
  morphologyBuilder.displayOnScene = function(scene, data, done, updateBoundingBox, getPointData, callbackMeshes) {
  ...
  morphologyBuilder.displayPointOnScene = function(scene, pointString) {
    ...
    sphere.position = new THREE.Vector3(positionData[0],
                                        positionData[1],
                                        positionData[2]);
    scene.add(sphere);
  };
  function displayMorphology(inputData, scene, doneCallback, bbCallback, getPointData, callbackMeshes) {
    ...
    var meshes = [];
    _.forEach(acceptableTypes, function(type) {
  ...
  function createSomaMesh(scene, allPoints){
    ...
    somaData.forEach(function(p) {
      averageSoma.set(averageSoma.x + p.x,
                      averageSoma.y + p.y,
                      averageSoma.z + p.z);
    });
  ...
  function createMesh(data, scene, allPoints, updateBoundingBox, type){
    ...
    for (var idx = 0 ; idx < data.length; idx++){
      ...
      if (parentPt.type == NeuronPartType.SOMA){continue;}
...
module.exports = {
  'morphologyBuilder': morphologyBuilder
};
